 ...
 * @since 1.1.0
 */
public class MyBatisCustomWriter<T> implements ItemWriter<List<T>>, InitializingBean {

  private static final Logger LOGGER = LoggerFactory.getLogger(MyBatisCustomWriter.class);
  ...
  public void write(final List<? extends List<T>> items) {
    ...
    if (!items.isEmpty()) {
      ...
      for (List<T> item : items) {
    	  for(T t: item)
    		  sqlSessionTemplate.update(statementId, itemToParameterConverter.convert(t));
      }
